...
"""
def Xpander_topo_with_host(switch_num, d, hosts_per_switch=1, total_hosts=256, host_bw=1, sw2sw_bw=1, 
                        topo_dir="topo.txt", trace_dir="", mat_dir="", 
                        ecmp_topo_dir = "", ecmp_trace_dir="", ecmp_mat_dir=""):
  ...
  with open(ecmp_trace_dir, 'w') as f:
    f.write("%d\n" % len(node_ids))
    f.write("%s\n" % (" ".join(map(str, node_ids))) )
  ...
  """ ecmp topo generate """
  topo = open(ecmp_topo_dir, mode='w')
  topo.write("%d %d %d\n" % (total_nodes, switch_num, link_num) )
  topo.write("%s\n" % (" ".join(map(str, switch_ids))))

  """step1: ecmp connect host with switch"""
  for host_id in host_ids:
    topo.write("%d %d %.2fGbps %dns 0.000000\n" % (host_id, switch_ids[host_id // hosts_per_switch ], host_bw, 1000))
  
  """step 2: ecmp connect switch with switch"""
  for i in range(switch_num):
    for j in range(switch_num):
      if i > j:
        if mat[i, j] == 1:
...
if __name__ == "__main__":
  ...
  mat_fmt = "../experiment/topo/Xpander/%s/mat.txt"
  topo_fmt = "../experiment/topo/Xpander/%s/topo.txt"
  trace_fmt = "../experiment/trace/Xpander/%s/trace.txt"

  ecmp_mat_fmt = "../experiment/topo/Xpander-ecmp/%s/mat.txt"
  ecmp_topo_fmt = "../experiment/topo/Xpander-ecmp/%s/topo.txt"
  ecmp_trace_fmt = "../experiment/trace/Xpander-ecmp/%s/trace.txt"
  ...
  Xpander_topo_with_host(32, 
              ...
              mat_dir=mat_fmt % ("7:8"),
              ecmp_topo_dir=ecmp_topo_fmt % ("7:8"),
              ecmp_trace_dir=ecmp_trace_fmt % ("7:8"),
              ecmp_mat_dir=ecmp_mat_fmt % ("7:8"))
  Xpander_topo_with_host(48, 
              ...
              mat_dir=mat_fmt % ("7:6"),
              ecmp_topo_dir=ecmp_topo_fmt % ("7:6"),
              ecmp_trace_dir=ecmp_trace_fmt % ("7:6"),
              ecmp_mat_dir=ecmp_mat_fmt % ("7:6"))
  Xpander_topo_with_host(64, 
              ...
              mat_dir=mat_fmt % ("7:4"),
              ecmp_topo_dir=ecmp_topo_fmt % ("7:4"),
              ecmp_trace_dir=ecmp_trace_fmt % ("7:4"),
              ecmp_mat_dir=ecmp_mat_fmt % ("7:4"))