...
void ReadFlowInput(){
	if (flow_input.idx < flow_num){
		...
		//std::cout << "Read flow input: " << flow_input.idx << std::endl;
...
int main(int argc, char *argv[])
{
	...
	std::cout << "Flow number: " << flow_num 
			<< "\nNode num: " << node_num
			<< "\nLink num: " << link_num
			<< "\nSwitch num: " << switch_num 
			<< "\nTrace num: " << trace_num << std::endl;
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // is switch
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++){
				...
				//std::cout << "Kmax map rate: " << rate << std::endl;
	...
	SetRoutingEntries();
	ed = clock();
	std::cout << "Bfs for routing calculate: " << (ed - bg) / CLOCKS_PER_SEC << "s" << "\n";
	//
	// get BDP and delay
	//
	maxRtt = maxBdp = 0;
