...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
    ...
    .AddAttribute ("BaseRtt",
                   ...
                   MakeUintegerChecker<uint64_t> ())
   .AddAttribute ("NonShortest", 
                "Non shortest path", 
                UintegerValue(0),
                MakeUintegerAccessor (&RdmaClient::m_nonShortest),
                MakeUintegerChecker<uint32_t> ())
  .AddAttribute ("EnableSack",
                "enable sack",
                BooleanValue(false),
                MakeBooleanAccessor(&RdmaClient::m_enableSack),
                MakeBooleanChecker())
  .AddAttribute ("EnableIRN",
                "enable IRN",
                BooleanValue(false),
                MakeBooleanAccessor(&RdmaClient::m_enableIRN),
                MakeBooleanChecker())
  .AddAttribute ("EnablePfc",
                "enable PFC",
                BooleanValue(true),
                MakeBooleanAccessor(&RdmaClient::m_enablePfc),
                MakeBooleanChecker())
  ;
  return tid;
}
...
void RdmaClient::StartApplication (void)
{
  ...
  // std::cout << "Enable IRN in rdma client: " << m_enableIRN << "\n";
  rdma->AddQueuePair(m_size, 
                      m_pg, 
                      m_sip, 
                      m_dip, 
                      m_sport, 
