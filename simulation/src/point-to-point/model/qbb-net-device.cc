...
// #include "ns3/selective-seg-header.h"
...
namespace ns3 {
	...
	RdmaEgressQueue::RdmaEgressQueue(){
		...
		m_lossRecoverQ = CreateObject<DropTailQueue>();
	...
	Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex){
		...
		if (qIndex >= 0){ // qp
			...
			// loss recovery
			{
				Ptr<Packet> p = m_lossRecoverQ->Dequeue();
				// if dequeue failed, it returns 0
				if (p != 0) {
					m_rrlast = qIndex;
					m_qlast = qIndex;
	...
	void RdmaEgressQueue::EnqueueLossRecoverQ(Ptr<Packet> p) {
		m_traceRdmaEnqueue(p, 0);
		m_lossRecoverQ->Enqueue(p);
	}
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddAttribute ("EnableSack",
				"enable sack or not.",
				BooleanValue(false),
				MakeBooleanAccessor(&QbbNetDevice::m_enableSack),
				MakeBooleanChecker())
			;
	...
	// void QbbNetDevice::SetStatus(NetDevStatus status) {
	// 	m_status = status;
	// }

	// enum NetDevStatus QbbNetDevice::GetStatus() {
	// 	return m_status;
	// }
	...
	{
		NS_LOG_FUNCTION(this);
		//std::cout << "Before transmit complete: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state" << m_txMachineState  << " " << Simulator::Now().GetTimeStep() << "\n";
		NS_ASSERT_MSG(m_txMachineState == BUSY, "Must be BUSY if transmitting");
		m_txMachineState = READY;
		NS_ASSERT_MSG(m_currentPkt != 0, "QbbNetDevice::TransmitComplete(): m_currentPkt zero");
		m_phyTxEndTrace(m_currentPkt);
		m_currentPkt = 0;
		//std::cout << "After transmit complete: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state" << m_txMachineState  << " " << Simulator::Now().GetTimeStep() << "\n";
		// ////std::cout << "Transcomplete Dequeue()." << std::endl;
		DequeueAndTransmit();
	}
	...
	{
		NS_LOG_FUNCTION(this);
		if (m_inDequeueAndTransmit) return;
		if (!m_linkUp) return; // if link is down, return
		if (m_txMachineState == BUSY) return;	// Quit if channel busy
		m_inDequeueAndTransmit = true;
		NS_ASSERT_MSG(m_txMachineState == READY, "ensure ready in dequeueandtransmit");
		//std::cout << "In queueand transmit : " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state: " << m_txMachineState <<  " " << Simulator::Now().GetTimeStep() << "\n";
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){
			...
			NS_ASSERT_MSG(m_txMachineState == READY , "switch dequeue");
			p = m_queue->DequeueRR(m_paused);		//this is round-robin
			NS_ASSERT_MSG(m_txMachineState == READY, "ensure ready in switchpart");
			if (p != 0){
				//std::cout << "test dequeandtransmit\n";
				m_snifferTrace(p);
				...
				//std::cout << "before switch: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state: " << m_txMachineState <<  " " << Simulator::Now().GetTimeStep() << "\n";
				// ////std::cout << p << "\n";
				NS_ASSERT_MSG(m_txMachineState == READY, "ensure ready in dequeueandtransmit");
				...
				// }
				TransmitStart(p);
				//std::cout << "After switch: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state: " << m_txMachineState << " " << Simulator::Now().GetTimeStep() << "\n";
				m_inDequeueAndTransmit = false;
				return;
		...
		QbbNetDevice::Resume(unsigned qIndex)
	{
		...
		// ////std::cout << "Resume Dequeue. " << std::endl;
		//std::cout << "Switch resume: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state: " << m_txMachineState << " " << Simulator::Now().GetTimeStep() << "\n";
		DequeueAndTransmit();
	}
		...
		if (ch.l3Prot == 0xFE){ // PFC
			...
			if (ch.pfc.time > 0){
				...
				if (m_node->GetNodeType() > 0) { // switch
					DynamicCast<SwitchNode>(m_node)->SwitchSetPNCounter(m_ifIndex, qIndex);
				}
			}
	...
	bool QbbNetDevice::SwitchSend (uint32_t qIndex, Ptr<Packet> packet, CustomHeader &ch){
		...
		// std::cout << "QbbNetDev switchSend\n";
		DequeueAndTransmit();
		// std::cout << "sip: " << ch.sip << " dip: " << ch.dip << " now: " << Simulator::Now().GetTimeStep() << "\n";
		return true;
	}
	...
	void QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type){
		...
		CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
		p->PeekHeader(ch);
		//std::cout << "send pfccccccccccccccccccccccc\n";
		SwitchSend(0, p, ch);
	}
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		////std::cout << "Transmit start: " << m_node->GetId() << " ifidx: " << m_ifIndex << "m_state: " << m_txMachineState << " " << Simulator::Now().GetTimeStep() << "\n";
		//
   ...
   void QbbNetDevice::NewQp(Ptr<RdmaQueuePair> qp){
	   qp->m_nextAvail = Simulator::Now();
		 //std::cout << "Newqp Dequeue()." << std::endl;
	   DequeueAndTransmit();
   }
   void QbbNetDevice::ReassignedQp(Ptr<RdmaQueuePair> qp){
	   //std::cout << "ReassignedQp Dequeue()." << std::endl;
		 DequeueAndTransmit();
   }
   void QbbNetDevice::TriggerTransmit(void){
	   //std::cout << "TriggerTransmit Dequeue()." << std::endl;
	   DequeueAndTransmit();
   }
	...
	void QbbNetDevice::RdmaEnqueueLossRecoveryQ(Ptr<Packet> p) {
		m_traceEnqueue(p, 3);
		m_rdmaEQ->EnqueueLossRecoverQ(p);
	}
	...
	void QbbNetDevice::UpdateNextAvail(Time t){
		if (!m_nextSend.IsExpired() && t < m_nextSend.GetTs()){
			...
			//std::cout << "Update next available Dequeue()." << std::endl;
