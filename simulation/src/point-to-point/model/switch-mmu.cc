...
namespace ns3 {
	...
	SwitchMmu::SwitchMmu(void){
		...
		memset(PNCounter, 0, sizeof(PNCounter));
		memset(pktNumInQueue, 0, sizeof(pktNumInQueue));
	}
	void SwitchMmu::ConfigAlpha(double _alpha) {
		alpha = _alpha;
	}
	uint32_t SwitchMmu::GetIngressBytes(uint32_t port, uint32_t qIndex) {
		return ingress_bytes[port][qIndex];
	}
	bool SwitchMmu::CheckShouldBypass(uint32_t port, uint32_t qIndex) {
		// 检查流向该端口的流是否需要分流，也就是不走最短路径
		if (GetSharedUsed(port, qIndex) >= GetPfcThreshold(port) * alpha) {
			return true;
		}
		return false;
	}
	...
	void SwitchMmu::UpdateIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		if (new_bytes <= reserve){
			...
			if (new_bytes - reserve > thresh){
				...
				// update bytes in ingress port.
				// such packet will stored in shared memory
				ingress_bytes[port][qIndex] += psize;
	...
	void SwitchMmu::UpdateEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		// PCN 
		pktNumInQueue[port][qIndex] += 1;
		// if (node_id == 275)
		// std::cout << node_id << " qIdx: " << qIndex 
		// 	<< " port: " << port << " bytes: " << egress_bytes[port][qIndex] << " qlen: " << pktNumInQueue[port][qIndex] << "\n";
	}
	...
	void SwitchMmu::RemoveFromEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		egress_bytes[port][qIndex] -= psize;
		// if (pktNumInQueue[port][qIndex] > 0)
			pktNumInQueue[port][qIndex] -= 1;
		if (PNCounter[port] > 0)
			PNCounter[port] -= 1;
	}
	 ...
	 * PCN ECN marking
	 ******************/
	bool SwitchMmu::PcnSendCN(uint32_t ifIndex, uint32_t qIndex) {
		if (qIndex == 0) 
			return false;
		if (PNCounter[ifIndex] == 0 &&  pktNumInQueue[ifIndex][qIndex] > 0) 
			return true;
		return false;
	}
	void SwitchMmu::SetPNCounter(uint32_t ifIndex, uint32_t qIndex) {
		// if (node_id == 275)
		// 	std::cout << node_id << " qIdx: " << qIndex 
		// 	<< " port " << ifIndex << " egress bytes: " << egress_bytes[ifIndex][qIndex] << " SetPNCounter: " << pktNumInQueue[ifIndex][qIndex] << "\n";
		PNCounter[ifIndex] = pktNumInQueue[ifIndex][qIndex];
	}
	...
	void SwitchMmu::ConfigEcn(uint32_t port, double _kmin, double _kmax, double _pmax) {
		kmin[port] = _kmin * 1024;
		kmax[port] = _kmax * 1024;
		pmax[port] = _pmax;
		// std::cout << "ConfigEcn: " << kmin[port] << " " << 	kmax[port] << "\n";
	}
