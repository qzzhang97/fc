...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("PintSmplThresh",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("PcnWMin",
				"PCN's WMin for rate adjustment",
				DoubleValue(1/256),
				MakeDoubleAccessor(&RdmaHw::m_wMin),
				MakeDoubleChecker<double>())
		.AddAttribute("PcnWMax",
				"PCN's WMax for rate adjustment",
				DoubleValue(0.5),
				MakeDoubleAccessor(&RdmaHw::m_wMax),
				MakeDoubleChecker<double>())
		.AddAttribute("EnableSack",
				"enable sack or not",
				BooleanValue(false),
				MakeBooleanAccessor(&RdmaHw::m_enableSack),
				MakeBooleanChecker())
		;
	return tid;
}
...
void RdmaHw::Setup(QpCompleteCallback cb, PktCompleteCallback pkt_cb){
	for (uint32_t i = 0; i < m_nic.size(); i++){
		...
		// config NIC
		if (m_enableSack) { // IRN send packet logic
			dev->m_rdmaEQ->m_rdmaGetNxtPkt = MakeCallback(&RdmaHw::GetNxtPacketIRN, this);
		} else { // pfc send packet logic
			dev->m_rdmaEQ->m_rdmaGetNxtPkt = MakeCallback(&RdmaHw::GetNxtPacket, this);
		}
	}
	// setup qp complete callback
	m_qpCompleteCallback = cb;
	m_pktCompleteCb = pkt_cb;
}

uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp){
	auto &v = m_rtTable[qp->dip.Get()];

	// std::cout << "s: " << ((qp->sip.Get() >> 8) & 0xffff)<< " d: " << ( (qp->dip.Get() >> 8) & 0xffff ) <<  "\n";
	// std::cout << "qp v.size: " << v.size() << std::endl;
	if (v.size() > 0){
		return v[qp->GetHash() % v.size()];
	}else{
		std::cout << qp->dip.Get() << std::endl;
...
void RdmaHw::AddQueuePair(
	...
	//added by qizhou.zqz
	qp->SetNonShortest(non_shortest);
	qp->m_enableSack = _enable_sack;
	qp->m_enableIRN = _enable_irn;
	qp->m_enablePfc = _enable_pfc;
	// std::cout << "enable pfc in mesh: " << _enable_pfc << "\n";
	// add qp
	...
	// std::cout << "Node ID " << m_node->GetId() << " NIC_idx: " << nic_idx << "\n";
	// std::cout << m_nic[nic_idx].dev << "\n";
	// set init variables
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, bool create){
	...
	if (create){
		...
		if (m_cc_mode == 22) {
			q->m_firstPktRxQpRecv = true;
		}

		// store in map
		m_rxQpMap[key] = q;
		// std::cout << "create rxqp: " << m_rxQpMap.size() << "\n";
		return q;
	}
	return NULL;
}
...
void RdmaHw::DeleteRxQp(uint32_t dip, uint16_t pg, uint16_t dport){
	uint64_t key = ((uint64_t)dip << 32) | ((uint64_t)pg << 16) | (uint64_t)dport;
	Ptr<RdmaRxQueuePair> rxQp = m_rxQpMap[key];
	// Simulator::Cancel(rxQp->PcnTimerEvent);
	m_rxQpMap.erase(key);
}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	/******************************************************
	 * Behavior of RxQp in PCN mode.
	 * We set m_cc_mode equal to 22 to represent the CC is PCN
	 ******************************************************/
	if (m_cc_mode == 22) {
		if (rxQp->m_firstPktRxQpRecv) {
			rxQp->m_NpVar.recNum = 1;
			rxQp->m_NpVar.recData = p->GetSize();
			rxQp->m_NpVar.interArrivalTime = 0; 
			rxQp->m_NpVar.pktLastArrivalTime = Simulator::Now().GetTimeStep();
			if (ecnbits != 0) {
				rxQp->m_NpVar.ecnNum = 1;
			}
			rxQp->m_firstPktRxQpRecv = false;
			rxQp->PcnTimerEvent = Simulator::Schedule(MicroSeconds(50.0), &RdmaRxQueuePair::PcnNpStateChange, rxQp, this);
		} else {
			rxQp->m_NpVar.recNum++;
			rxQp->m_NpVar.recData += p->GetSize();
			rxQp->m_NpVar.interArrivalTime = Simulator::Now().GetTimeStep() - rxQp->m_NpVar.pktLastArrivalTime; 
			rxQp->m_NpVar.pktLastArrivalTime = Simulator::Now().GetTimeStep();
			if (ecnbits != 0) {
				rxQp->m_NpVar.ecnNum++;
			}
			...
			// std::cout <<"rec_num : " << rxQp->m_NpVar.recNum 
			// << " rec_ecn: " << rxQp->m_NpVar.ecnNum++ 
			// << " rec_data: " << rxQp->m_NpVar.recData << "\n";
		}
	...
	if (x == 1 || x == 2){ //generate ACK or NACK
		...
		if (m_enableSack) {
			...
			sackHeader.SetBlockNum(rxQp->m_sackSet.GetBlockNum());
			// std::cout << "Block Num: " << rxQp->m_sackSet.GetBlockNum() << "\n";
			sackHeader.SetBlocks(rxQp->m_sackSet.GetBlocks());
			// std::cout << "Set SACK BLOCK HEADER done\n";
			// std::cout << "Dump SACK block: " << rxQp->m_sackSet.toString() << "\n";
			qbbHeader seqh;
			...
			uint32_t	psize;
			if (x == 1) {
				psize = std::max(60-14-20-(int)seqh.GetSerializedSize(), 0);
				// std::cout << "ack psize= " << psize << "\n";
			} else if (x == 2) {
				// std::cout << "trigger NACK\n";
				// std::cout << "ch.sip: " << ((ch.sip) >> 8 & 0xffff) << "ch.dip: " 
				// 	<< ((ch.sip) >> 8 & 0xffff) << "blocks: " 
				// 	<< numBlocks << rxQp->m_selectiveAckRangeSet.toString() << "\n";
				psize = std::max(60-14-20-(int)seqh.GetSerializedSize() - (int)sackHeader.GetSerializedSize(), 0);
				// std::cout << "seqh size = " << seqh.GetSerializedSize() << "\n";
				// std::cout << "generate NACK... 409 in hw.cc: " << selectiveHeader.GetSerializedSize() << " ack size: " << psize << "\n";
			}
			

			Ptr<Packet> newp = Create<Packet>(psize);
			if (x == 2) {
				newp->AddHeader(sackHeader);
			  // std::cout << "genrate NACK rcvExpectedSeq " << rxQp->ReceiverNextExpectedSeq << "ch.sip = " << ((ch.sip >> 8) & 0xffff) << " "<< "generate NACK " << rxQp->m_selectiveAckRangeSet.toString() << "\n";
			}
			newp->AddHeader(seqh);
			...
			//
			// std::cout << "send ack or nack done in hw.cc 445\n";
		} else {
			//
			qbbHeader seqh;
			seqh.SetSeq(rxQp->ReceiverNextExpectedSeq);
...
int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch){
	...
	// std::cout << ch.sip << " " << udpport << " " << qIndex << "\n";
	// std::cout<< "In receive Cnp m_qpMap size: " << m_qpMap.size() << "\n";
	if (qp == NULL) {
		// std::cout << "ERROR: QCN NIC cannot find the flow\n";
		return -1;
	}

	// handle cnp if congestion cotrol algorithm is PCN.
	if (m_cc_mode == 22) {
		cnp_received_pcn(qp, p, ch);
	}
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	if (m_enableSack) { // handle ACK or NACK in SACK mode
		// selective Ack
		qp->handleAckOrNack(p, ch);
		if (qp->IsFinished()){
			QpComplete(qp);
		}
	} else {
...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size){
	if (m_enableSack) { // for sack, receiver's action
		// receiver's expectedSequence equal to sender's snd_una variable
		uint32_t expected = q->ReceiverNextExpectedSeq;
		if (expected == seq) {
			q->ReceiverNextExpectedSeq = q->m_sackSet.DetermineRecvSeqNum(seq, size);
			return 1;
		} else if (seq > expected) {
			q->m_sackSet.add(seq, size);
			//SELECTIVE NACK
			// std::cout << "trigger NACK\n";
			return 2;
			// if (Simulator::Now() >= q->m_nackTimer || q->m_lastNACK != expected) {
			// 	// avoid frequently generating NACK
...
Ptr<Packet> RdmaHw::GetNxtPacketIRN(Ptr<RdmaQueuePair> qp) {
	//
	Ptr<Packet> p = qp->txFree(m_mtu);
	NS_ASSERT_MSG(p != nullptr, "packet should not be nullptr");
	return p;
} 
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	qp->m_ipid++;
	// update QP snd_nxt指针
	p->AddPacketTag(LongerPathTag(1));
	// qp->updateSendQpState(payload_size);
	return p;
}
 ...
 *   PCN
 ****************/
void RdmaHw::cnp_received_pcn(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
	UpdateRatePcn(qp, p, ch);
}

void RdmaHw::UpdateRatePcn(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
	// rate adjust
	DataRate sendRate = qp->m_rate;
	DataRate recRate(ch.cnp.recRate * 1e6);
	// std::cout << "rec: " << ch.cnp.recRate << " rec * 1e6: " << ch.cnp.recRate * 1e6 << "\n";
	double w = qp->pcn.m_w;
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
	if (ch.cnp.ecnBits == 1) { //rate decrease
		// std::cout << "send " << sendRate << " rec * min = " << recRate * (1 - m_wMin)  << "  rate decrease\n";
		sendRate = std::min(sendRate, recRate * (1 - m_wMin));
		sendRate = std::max(m_minRate, sendRate);
		w = m_wMin;
	} else { // rate increase
		sendRate = sendRate * (1 - w) + dev->GetDataRate() * w;
		sendRate = std::min(sendRate, qp->m_max_rate);
		w = w * (1 - w) + m_wMax * w;
		// std::cout << "rate increase\n";
	}
	qp->pcn.m_w = w;
	qp->m_rate = sendRate;
}
