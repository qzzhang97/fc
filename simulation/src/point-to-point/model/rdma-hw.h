...
public:
	...
	typedef Callback<void, Ptr<Packet> > PktCompleteCallback;
	PktCompleteCallback m_pktCompleteCb;

	void SetNode(Ptr<Node> node);
	void Setup(QpCompleteCallback cb, PktCompleteCallback pkt_cb); // setup shared data and callbacks with the QbbNetDevice
	...
	Ptr<Packet> GetNxtPacketIRN(Ptr<RdmaQueuePair> qp);
	...
	double m_wMin;
	double m_wMax;
	void cnp_received_pcn(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void UpdateRatePcn(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);

	/****************
	 * SACK method
	 ****************/
	// Ptr<Packet> createNackPacket(Ptr<RdmaRxQueuePair> rxQp);
	// Ptr<Packet> createAckPacket(Ptr<RdmaRxQueuePair> rxQp);
};
