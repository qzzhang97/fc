...
public:
  ...
  Ptr<DropTailQueue> m_lossRecoverQ;

  void EnqueueLossRecoverQ(Ptr<Packet> p);
...
public:
  ...
  //Added by qizhou.zqz!
  // void SetStatus(NetDevStatus status);

  // enum NetDevStatus GetStatus();

  bool QueuePaused(int qIndex) {
...
public:
  bool m_inDequeueAndTransmit;
  bool m_enableSack;
	Ptr<RdmaEgressQueue> m_rdmaEQ;
	void RdmaEnqueueHighPrioQ(Ptr<Packet> p);
  void RdmaEnqueueLossRecoveryQ(Ptr<Packet> p);
