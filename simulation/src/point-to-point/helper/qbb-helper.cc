...
NetDeviceContainer 
QbbHelper::InstallRRG (Ptr<Node> a, Ptr<Node> b, uint32_t port1, uint32_t port2)
{
  NetDeviceContainer container;
  
  Ptr<QbbNetDevice> devA = m_deviceFactory.Create<QbbNetDevice> ();
  devA->SetAddress (Mac48Address::Allocate ());
  // devA->SetIfIndex(port1);
  a->SetDeviceAtIndex (devA, port1);

  Ptr<QbbNetDevice> devB = m_deviceFactory.Create<QbbNetDevice> ();
  devB->SetAddress (Mac48Address::Allocate ());
  // devB->SetIfIndex(port2);
  b->SetDeviceAtIndex (devB, port2);
  ...
  Ptr<QbbChannel> channel = m_channelFactory.Create<QbbChannel> ();
