...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L3_Header){
	  ...
	  if (brief){
		  m_tos = i.ReadU8 ();
		//   std::cout << "In customheader.cc line 218: m_tos=" << m_tos << "\n";
		  i.Next(2);
		  ipid = i.ReadNtohU16();
		  i.Next(3);
		  l3Prot = i.ReadU8();
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  if (getInt)
			...
			//
			if (l3Prot == 0xFC || l3Prot == 0xFD) { // ACK
				l4Size = GetAckSerializedSize();
				if (enable_sack && l3Prot == 0xFD) {
					ack.sackHeader.Deserialize(i);
...
uint8_t CustomHeader::GetIpv4DscpValue (void) const {
	return m_tos & 0xFC;
}
...
uint32_t CustomHeader::GetNackSerializedSize(void) {
	return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) + sizeof(ack.seq) 
					+ IntHeader::GetStaticSize() + SackHeader::GetStaticSize();
}
...
uint8_t CustomHeader::GetMyTos(void) const{
	return m_tos;
}
