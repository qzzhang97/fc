...
Node::AddDevice (Ptr<NetDevice> device)
{
  ...
  // std::cout << GetId() << " dev index: " << index << "\n";
  m_devices.push_back (device);
  device->SetNode (this);
  // index is devices.size()
...
void Node::SetDeviceAtIndex (Ptr<NetDevice> device, uint32_t index) {
  // std::cout << "index= " << index << ", " << m_devices.size() << "\n";
  NS_ASSERT_MSG(index >= 0 && index < m_devices.size(), "index must be bounded");
